...
- 📚 SOURCED (2026-07-29) Drag-and-drop is a second grant path: `DataTransferItem.getAsFileSystemHandle()` yields a directory handle with read permission, no picker dialog. Added after managed-Chrome content-analysis scans hung the picker flow (org policy refresh 2026-07-28; a `chrome://restart` cleared it once, then it recurred). → Both the pick page and the panel accept a dropped folder (`lib/drop.js`); the picker remains the fallback. Drop-path scan behavior under the org connector: ❌ tested 2026-07-29 — enterprise content analysis ("Checking these files with your organization's security policies…") fires on drop-based grants too; it gates the grant itself, not the picker dialog. Drop remains for UX (no dialog round-trip) but is not a policy bypass; on managed profiles all new-folder grants are subject to the org scan.
- ✅ VERIFIED (2026-07-29) The hanging scan is directory-specific: Chrome's own single-file `showOpenFilePicker` demo cleared the same org connector (`OnFileAttachedEnterpriseConnector`, Cloud/Google) in ~1s, while a 604 KB / 18-entry **directory** grant hung >2min. The connector returns verdicts for file grants but wedges on directory-shaped ones. → **Files mode**: `showOpenFilePicker({multiple})` picks markdown files individually (each cleared by the fast per-file scan); stored as a handle array. Flat list, no cross-file relative-link resolution unless the target was also picked — the documented trade-off for working under the policy. Directory mode stays the default.
...
**Components:**
- `panel.html/js` — tree + reader, grant lifecycle (restore → query/requestPermission; picking delegated to the pick tab, result received via BroadcastChannel). Stored grant is either a directory handle or an array of file handles.
- `reader.html/js` — full-tab reader, `?path=` param, same modules; also the pick page (`?pick=1`) offering folder or files (see research log).
- `lib/source.js` — `createRealSource` (directory), `createFilesSource` (flat file-handle set), `createMockSource`; uniform `{name, tree, read, blob}` so the panel/reader/viewer don't branch on grant type.
